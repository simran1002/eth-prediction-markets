...
import {
  useApprovePositionPurchase,
  useBuyPosition,
  usePositionPurchaseAllowance,
} from "@/lib/hooks/usePositionTrading";
...
function PositionListingCard({
  ...
  const { buyPosition, isPending: isBuying } = useBuyPosition();
  const {
    approve,
  ...
  const marketTuple = market as readonly [
    bigint,
    string,
    string,
    readonly string[],
  ...
  const tokenAddress = marketTuple?.[9];
  const isEthMarket =
    !tokenAddress || tokenAddress === zeroAddress;

  const { hasEnoughAllowance, refetch: refetchAllowance } =
    usePositionPurchaseAllowance(
      isEthMarket ? undefined : (tokenAddress as `0x${string}`),
      userAddress,
      listing.price
    );
  ...
  useEffect(() => {
    if (isApproveSuccess) {
      refetchAllowance();
      toast.success("USDC approved. You can buy this position.");
    }
  }, [isApproveSuccess, refetchAllowance]);

  if (!market || !marketTuple) {
    return (
  ...
  const [, question, , outcomes] = marketTuple;
  ...
  const needsApproval = !isEthMarket && !hasEnoughAllowance;

  const handleApprove = () => {
    if (!tokenAddr || isEthMarket) return;
    approve(tokenAddr as `0x${string}`, formatEther(listing.price));
    toast.info("Approving USDC for purchase...");
  };

  const handleBuy = () => {
    if (needsApproval) {
      toast.error("Approve USDC before buying this position");
      return;
    }
    buyPosition(
      Number(listing.betId),
      formatEther(listing.price),
      tokenAddr as `0x${string}` | undefined
    );
        ...
        {isOwnListing ? (
          ...
          <div className="space-y-2">
            ...
            <Button
              className="w-full"
              onClick={handleBuy}
              disabled={isBuying || needsApproval}
            >
              {isBuying ? "Purchasing..." : `Buy for ${formattedPrice}`}
            </Button>
          </div>
        )}
      </CardContent>
